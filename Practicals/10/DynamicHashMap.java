...
public class DynamicHashMap {
     ...
     * @param tSize - the number of cells in the table or the maximum number of chain that can be in the table
    ...
    public int hash(String key) {
        int hashKey = key.charAt(0);
        for (int index=1;       index<key.length();     index++)
            hashKey = (hashKey ^ key.charAt(index));
        return hashKey % getTableSize();
    }
    ...
    public Integer get(String key) {
        ...
        return (keys.get(hashKey).contains(key)) ? keys.get(hashKey).indexOf(key) : null;
    }
